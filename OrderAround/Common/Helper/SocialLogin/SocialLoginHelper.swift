...
class SocialLoginHelper: NSObject, GIDSignInDelegate {
    //GIDSignInUIDelegate
    // MARK: - Declarations
                ...
                if fbloginresult.grantedPermissions != nil {
                    if(fbloginresult.grantedPermissions.contains("email")) {
                        ...
                        if(AccessToken.current != nil){
                            ...
                            print("ACCESS TOKEN FOR FB: \(AccessToken.current?.tokenString)")
                            GraphRequest(graphPath: "me", parameters: ["fields": "name,email,first_name, last_name, picture.type(large)"]).start(completionHandler: { (connection, result, error) -> Void in
                                if (error == nil){
    ...
    func loginThroughGoogle (fromViewController: UIViewController, helperDelegate: SocialLoginHelperDelegate) {
       ...
       //googleSignIn?.uiDelegate = self
        ...
        googleSignIn?.presentingViewController = self.mainViewController
        googleSignIn?.signIn()
    }
