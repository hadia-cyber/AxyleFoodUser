...
class DishesListCell: UITableViewCell {
    ...
    func setDesign()  {
        ...
        self.lblViewFullMenu.text = APPLocalize.localizestring.ViewFulMenu.localize()
        self.btnAdd.setTitle(APPLocalize.localizestring.add.localize(), for: .normal)
    ...
    func set(values:Products) {
        ...
        lblPrice.text =  String(format: "%.2f", (values.prices?.price ?? 0)) + (values.prices?.currency)!
