...
class MobileViewController: UIViewController {
    ...
    @IBOutlet weak var userText1: UILabel!
    
    @IBOutlet weak var usertext2: UILabel!
...
//        userText1.text = APPLocalize.localizestring.Avecnospartenaires.localize()
//        usertext2.text = APPLocalize.localizestring.trouvezvotrebonheur.localize()
        userText1.textColor = UIColor.primary
        usertext2.textColor = UIColor.primary
        countryCodeLbl.text = APPLocalize.localizestring.countryNumber.localize()
        countryFlagImg.image = UIImage(named: "CountryPicker.bundle/"+APPLocalize.localizestring.countryCode.localize())
        appleButton.addTarget(self, action: #selector(tapApple), for: .touchDown)
//        appleButton.isUserInteractionEnabled = trueååå´
        
    }
    
    @IBAction private func tapApple() {
          //        Apple SignIn
        if #available(iOS 13.0, *) {
            AppleSignin.shared.initAppleSignin(scope: [.email, .fullName]) { (appleData) in
                ...
                socialLogin.accessToken = appleData.userId ?? ""
                socialLogin.name = appleData.firstName ?? ""
                socialLogin.email = appleData.email ?? ""
                socialLogin.login_by = self.apple
    ...
    func design() {
        Common.setFont(to: mobileNumberTxtFlb, isTitle: true, size: 14, fontType: .regular)
         Common.setFont(to: userText1, isTitle: true, size: 20, fontType: .regular)
         Common.setFont(to: usertext2, isTitle: true, size: 20, fontType: .regular)
    ...
    private func constraints()
    {
        ...
        NSLayoutConstraint.activate([
            appleButton.topAnchor.constraint(equalTo: self.orLabel.topAnchor, constant: 25),
            appleButton.leadingAnchor.constraint(equalTo: self.appleSigninView.leadingAnchor, constant: 25),
            appleButton.trailingAnchor.constraint(equalTo: self.appleSigninView.trailingAnchor, constant: -25),
            appleButton.bottomAnchor.constraint(equalTo: self.orLabel.bottomAnchor, constant: 25),
...
//         socialLogin.loginThroughFacebook(fromViewController: self, helperDelegate: self)
    ...
    func getFBUserData()
    {
        if((AccessToken.current) != nil)
        {
            ...
            UserDefaults.standard.set(AccessToken.current?.tokenString, forKey: Keys.list.socialLoginAccessToken)
