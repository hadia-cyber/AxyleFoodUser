...
class PamentListViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        proceedToPayButton.setTitle(APPLocalize.localizestring.ProccedtoPay.localize(), for: .normal)
    ...
    @IBAction func moveToCheckPOut(_ sender: UIButton) {
      print(paymentMode)
        ...
        if paymentMode == PAYMENT_MODE().CASH {
            ...
            self.userOrderCheckOutInfo?.safe_delivery = 0
            ...
            self.userOrderCheckOutInfo?.safe_delivery = self.useDelivery
            if promoCodeId != 0 {
                self.userOrderCheckOutInfo?.promocode_id =  promoCodeId
