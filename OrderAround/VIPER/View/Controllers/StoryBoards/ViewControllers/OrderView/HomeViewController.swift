...
extension HomeViewController: PostViewProtocol {
    ...
    func getUserProfileDetails() {
        ...
        if User.main.accessToken != nil {
            ...
            self.presenter?.get(api: .userProfile, data: loginRequestData.toData())
        } else {
    ...
    func didReceiveSavedAddress(isUpdated: Bool?, addressDetails: UserAddressDetails?) {
        ...
        if isUpdated == true {
            DispatchQueue.main.async {
                self.locationLbl.text = addressDetails?.type?.uppercased()
                self.addressLbl.text = addressDetails?.map_address
                self.currentLocation.latitude = (addressDetails?.latitude)!
                self.currentLocation.longitude = (addressDetails?.longitude)!
                
                self.userLocation = UserCurrentLocation()
                self.userLocation?.latitude = self.currentLocation.latitude
                self.userLocation?.longitude = self.currentLocation.longitude
                self.userLocation?.user_id = User.main.id
