    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 1:
            ...
            if isFromFeaturedProducts == true {
                ...
                cell.priceLbl.text =    "\((featuredProducts?.prices?.orignal_price)!)" + (featuredProducts?.prices?.currency)!
                cell.descriptionLbl.text = featuredProducts?.description
                
            } else {
              if productDict != nil {
                cell.productName.text = productDict?.name
                cell.priceLbl.text =    "\((productDict?.prices?.orignal_price)!)" + (productDict?.prices?.currency)!
...
extension AddOnsViewController {
    @objc func minusButtonAction(sender: UIButton) {
        ...
        if isFromFeaturedProducts {
            ...
            self.cartItemLbl.text = APPLocalize.localizestring.perItem.localize()  + "\(totalPrice)" + (featuredProducts?.prices?.currency)!
            ...
            let totalPrice = totalItemPrice - price
            totalItemPrice = totalPrice
            self.cartItemLbl.text = APPLocalize.localizestring.perItem.localize() +   "\(totalPrice)" + (productDict?.prices?.currency)!
            for i in 0..<self.addOnsArray.count {
                let dictItem = self.addOnsArray[i] as! Dictionary<String,Any>
                let id =  dictItem["id"] as? Int ?? 0
                if id == dict?.id ?? 0 {
