...
class CartViewController: UIViewController {
    ...
    @IBAction func onschedlueBtnView(_ sender: Any) {
        ...
        vc.useDelivery = self.useDelivery
    ...
    @IBAction func onContinueBtnView(_ sender: Any) {
        ...
        paymentVC.useDelivery = self.useDelivery
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 2:
            ...
            if cartList.count > 0 && cartList.count > indexPath.row {
                ...
                let priceAmt =  String(format: " %.02f", Double(totalPriceAccount)) + String.removeNil(User.main.currency)
...
//                cell.useWalletButton.isHidden = true
                ...
                if walletAmt == 0{
                    ...
                    cell.useWalletLbl.text =   String(format: " %.2f", Double(walletAmt ?? 0)) + String.removeNil(User.main.currency)
            ...
            let totalPriceAmt = String(format: " %.02f", Double(totalPrice)) + String.removeNil(User.main.currency)

            cell.totalItemCountLbl.text = totalPriceAmt
            if let discountP = discountPrice {
                let discountPriceDouble = Double(discountP)
                let DiscountPriceAmt =  String(format: " %.02f", Double(discountPriceDouble!)) + String.removeNil(User.main.currency)

                cell.discountAmountLbl.text = "-" + DiscountPriceAmt

            }
            if let deliveryC = deliveryCharge {
                let deliveryChargeDouble =  String(format: " %.02f", Double(deliveryC)!) + String.removeNil(User.main.currency)
            ...
            cell.useWalletButton.addTarget(self, action: #selector(useWalletAmount), for: .touchUpInside)
               cell.useDeliveryButton.addTarget(self, action: #selector(useDeliveryButton(sender:)), for: .touchUpInside)
            cell.toPayAmountLbl.text = Common.showAmount(amount: Double(needToPay))
            cell.serviceAmountLbl.text =  String(format: " %.02f", Double(serviceTax)) + String.removeNil(User.main.currency)
            ...
            if promoCodeId == 0 {
                ...
                cell.applyButton.setTitle(APPLocalize.localizestring.applied.localize(), for: .normal)
...
extension CartViewController {
    ...
    @IBAction func useDeliveryButton(sender:UIButton){
       if let cell = tableView.cellForRow(at: IndexPath(row: 0, section: 3)) as? CartCheckOut {
        if cell.checkImage.image ==  #imageLiteral(resourceName: "uncheck")//imageLiteral(resourceName: "uncheck")
        {
            cell.checkImage.image = #imageLiteral(resourceName: "cellSelect")// imageLiteral(resourceName: "cellSelect")
            useDelivery = USE_DELIVERY().YES
            
        }
        else {
            
            cell.checkImage.image =  #imageLiteral(resourceName: "uncheck") //imageLiteral(resourceName: "uncheck")
            useDelivery = USE_DELIVERY().NO
    ...
    func addCart(api: Base, data: CartList?) {
        ...
        if api == .addCart, data != nil {
            ...
            if cartCount > 0 {
                ...
                deliveryCharge = String((data?.delivery_charges) ?? "0")
...
extension CartViewController: showDatePickerVCDelegate {
    func doneAction(isschedule: Bool, isscheduleDate: String, notes: String, useWallet: Int, addressId: Int, useDelivery: Int){
        let paymentVC = Router.main.instantiateViewController(withIdentifier: Storyboard.Ids.PamentListViewController) as! PamentListViewController
        ...
        paymentVC.useDelivery = useDelivery
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
